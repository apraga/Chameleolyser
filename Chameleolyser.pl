...
my 	$ALIGNMENT_FP									= "";
my	$NR_OF_THREADS									= 1;
my $GENOME = "GCA_000001405.15_GRCh38_full_analysis_set.fna";
...
sub MaskReferenceGenome {
	...
	my($STEM, $dirs, $suffix) = fileparse($RefGenome, qw(.fna .fa));
	my $MASKED = "$STEM.masked.fna";

	# Output is in REF
	system	("awk -v OFS='\t' {'print \$1,\$2'} $RefGenome.fai > $REF/$STEM.txt");
	system	("picard CreateSequenceDictionary REFERENCE=$RefGenome OUTPUT=$REF/$STEM.dict");
...
sub GenerateMaskedAlignmentAndVcf {
	...
	print "STEM: $STEM\n";
	my	$MaskedRefGenome		= "$WORKING_DIR/REF/$STEM.masked.fna";
	print $MaskedRefGenome;
	my	$GenomeFileFilePath		= "$WORKING_DIR/REF/$STEM.txt";

	my $ExtractionBedFP		= "$WORKING_DIR/BED/All.forextraction.bed";
	my $VarCallBedFP			= "$WORKING_DIR/BED/All.forvarcall.bed";
	my $CovExonsFP				= "$WORKING_DIR/BED/All.homologousexons.0.bed";
	my $RefGenome = "$WORKING_DIR/REF/$GENOME";
