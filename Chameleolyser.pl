...
sub MaskReferenceGenome {
	...
	system ("wget  https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/000/001/405/GCF_000001405.26_GRCh38/GRCh38_major_release_seqs_for_alignment_pipelines/GCA_000001405.15_GRCh38_full_analysis_set.fna.gz");
	system 	("gunzip GCA_000001405.15_GRCh38_full_analysis_set.fna.gz");
	system	("mv GCA_000001405.15_GRCh38_full_analysis_set.fna hg38.chr.fa");
	...
	system 	("bedtools maskfasta -fi hg38.chr.fa -bed ../BED/All.formasking.bed -fo hg38.masked.chr.fa");
...
sub GenerateMaskedAlignmentAndVcf {
	...
	$ExtractionBedFP		= "$WORKING_DIR/BED/All.forextraction.bed";
	$VarCallBedFP			= "$WORKING_DIR/BED/All.forvarcall.bed";
	$CovExonsFP				= "$WORKING_DIR/BED/All.homologousexons.0.bed";
...
sub FilterRawVariants {
	...
	foreach my $Chrom (sort keys %LoFreq_Variants){
		foreach my $Pos (sort keys %{$LoFreq_Variants{$Chrom}}){
			...
			#print $Chrom . ":" , $Pos . " with coverage ", $Coverage{$Chrom}{$Pos}, "\n";
			...
			foreach my $Variant (sort keys %{$LoFreq_Variants{$Chrom}{$Pos}}){
				...
				if ($CoordinateMappings{"$Chrom\~$ToMapOnStart"} && $CoordinateMappings{"$Chrom\~$ToMapOnEnd"}){
    				...
    				print $Variant . " with coordinates mapping with mappedsed=", $MappedSDs{$Variant}, "\n";
